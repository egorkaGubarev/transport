...
def create_slack_matrix(name, rows, limits, name_to_index, index_to_name):
    matrix = []
    prefix = name + '_'
    for i in range(rows):
        matrix.append(create_vector(prefix + str(i), utils.count_slack_amount(limits[i]), name_to_index, index_to_name))
    return matrix
